...
from verl.models.multi_resolution_reward import MultiResolutionVideoRewardModel
from verl.utils.multi_resolution_reward_dataset import MixedProgressWindowDataset, ProgressWindowDataset
...
class RobWMActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.config.model.vla == "openvla-oft":
                ...
                configured_statistics_path = self.config.model.get("dataset_statistics_path", None)
                if not os.path.isfile(dataset_statistics_path) and configured_statistics_path:
                    dataset_statistics_path = configured_statistics_path
    ...
    def _use_multi_resolution_reward(self):
        return bool(self.config.get("use_multi_resolution_reward", self.config.wm.get("use_multi_resolution_reward", False)))

    def _require_dual_head_reward_checkpoint(self):
        reward_loc_cfg = self.config.wm.get("reward_loc", {})
        return bool(reward_loc_cfg.get("require_dual_head_checkpoint", True))

    def _validate_reward_checkpoint(self, state_dict):
        if not self._use_multi_resolution_reward():
            return
        is_dual = MultiResolutionVideoRewardModel.is_dual_head_checkpoint(state_dict)
        if is_dual:
            return
        if self._require_dual_head_reward_checkpoint():
            raise ValueError(
                "use_multi_resolution_reward=True but the reward checkpoint does not contain "
                "`loc_head.*` parameters. Please load a dual-head reward checkpoint, or explicitly "
                "set reward_loc.require_dual_head_checkpoint=False if you intentionally want legacy initialization."
            )
        print(
            "[reward_loc] warning: dual-head checkpoint not found; initializing loc_head from traj success row. "
            "This is only for debugging compatibility and should not be used for real recovery experiments.",
            flush=True,
        )

    def _build_reward_model(self, device):
        feature_extractor = VideoMAEFeatureExtractor.from_pretrained(
            "MCG-NJU/videomae-base",
            size=self.config.wm.reward.img_size,
        )
        rw_cfg = VideoMAEConfig.from_pretrained(
            "MCG-NJU/videomae-base",
            num_frames=self.config.wm.reward.clip_len,
            num_labels=2,
        )
        base_model = VideoMAEForVideoClassification.from_pretrained("MCG-NJU/videomae-base", config=rw_cfg).to(device)
        if self._use_multi_resolution_reward():
            rm_model = MultiResolutionVideoRewardModel.from_videomae_classifier(
                base_model,
        ...
        self._validate_reward_checkpoint(state_dict)
        if hasattr(rm_model, "load_reward_state_dict"):
            rm_model.load_reward_state_dict(state_dict, strict=False)
        else:
            rm_model.load_state_dict(state_dict, strict=strict)
        return rm_model, feature_extractor

    def _compute_multi_resolution_reward_losses(self, model, batch, device):
        reward_loc_cfg = self.config.wm.reward_loc
        ...
        if self._use_multi_resolution_reward():
            ...
            traj_logits = model(pixel_values=traj_clip).logits
        loss_done = nn.CrossEntropyLoss()(traj_logits, traj_label)

        if not self._use_multi_resolution_reward() or not reward_loc_cfg.enable:
            zero = loss_done.new_zeros(())
            return {
                "loss_total": loss_done,
        ...
        rank_early_logits = model(pixel_values=rank_early.reshape(-1, *rank_early.shape[2:]), head="loc").logits.reshape(batch_size, pair_count)
        rank_late_logits = model(pixel_values=rank_late.reshape(-1, *rank_late.shape[2:]), head="loc").logits.reshape(batch_size, pair_count)
        ...
        denom = rank_mask.sum().clamp_min(1.0) * pair_count
        loss_rank = (rank_loss_matrix * rank_mask).sum() / denom
        ...
        fail_logits = model(pixel_values=fail_tail.reshape(-1, *fail_tail.shape[2:]), head="loc").logits.reshape(batch_size, fail_k)
        fail_targets = torch.zeros_like(fail_logits)
        fail_loss_matrix = torch.nn.functional.binary_cross_entropy_with_logits(fail_logits.float(), fail_targets, reduction="none")
        ...
        lambda_smooth = float(reward_loc_cfg.lambda_smooth)
        ...
        loss_total = (
            loss_done
            + float(reward_loc_cfg.lambda_anchor) * loss_anchor
            + float(reward_loc_cfg.lambda_rank) * loss_rank
            + float(reward_loc_cfg.lambda_fail) * loss_fail
            + float(reward_loc_cfg.lambda_smooth) * loss_smooth
        )
        return {
    ...
    @torch.no_grad()
    def _evaluate_loc_reward_model(self, dataloader):
        ...
        for batch in dataloader:
            ...
            rank_late = batch["rank_late_clips"].to(device, non_blocking=True)
            batch_size, pair_count = rank_early.shape[:2]
            early_logits = model(pixel_values=rank_early.reshape(-1, *rank_early.shape[2:]), head="loc").logits.reshape(batch_size, pair_count)
            late_logits = model(pixel_values=rank_late.reshape(-1, *rank_late.shape[2:]), head="loc").logits.reshape(batch_size, pair_count)
            ...
            curve_bins = curve_clips.shape[1]
            curve_logits = model(pixel_values=curve_clips.reshape(-1, *curve_clips.shape[2:]), head="loc").logits.reshape(batch_size, curve_bins)
        ...
        if curve_sum_local is None:
            curve_sum_local = torch.zeros(int(self.config.wm.reward_loc.progress_curve_bins), device=device, dtype=torch.float32)
        ...
        rank_acc = float(rank_tensor[0].item() / max(rank_tensor[1].item(), 1.0))
        tail_neg_score = float(tail_tensor[0].item() / max(tail_tensor[1].item(), 1.0))
        curve_count = max(curve_count_tensor[0].item(), 1.0)
        progress_curve = (curve_sum_local / curve_count).detach().cpu().tolist()
        return {
            "ranking_accuracy": rank_acc,
            "tail_negative_score": tail_neg_score,
            "progress_curve": [float(x) for x in progress_curve],
        }
    ...
    def _build_world_model(self):
        ...
        if not update_wm:
            ...
            rm_model, feature_extractor = self._build_reward_model(device)
            ...
            world_model_mapping = {
                ...
                "use_multi_resolution_reward": self._use_multi_resolution_reward(),
            }
    ...
    def _build_terminal_model_dataloader(self, global_steps=0):
        ...
        weights = [0.7, 0.3]
        loc_val_dataloader = None
        if self._use_multi_resolution_reward():
            dataset = MixedProgressWindowDataset(
                rollout_shards,
                expert_shards,
                weights,
                clip_len=self.config.wm.reward.clip_len,
                stride=self.config.wm.reward.clip_len,
                img_size=self.config.wm.reward.img_size,
                rank_num_pairs=self.config.wm.reward_loc.rank_num_pairs,
                failed_tail_k=self.config.wm.reward_loc.failed_tail_k,
                progress_curve_bins=self.config.wm.reward_loc.progress_curve_bins,
            )
        else:
            dataset = MixedSuccessWindowDataset(
                ...
                expert_shards,
                weights,
                self.config.wm.reward.clip_len,
                self.config.wm.reward.clip_len,
                self.config.wm.reward.img_size,
                mode="train",
            )
        dataloader = DataLoader(
            dataset,
        ...
        if self._use_multi_resolution_reward():
            ...
            loc_val_dataloader = DataLoader(
                loc_val_dataset,
                self.config.wm.rm_val_batch_size,
                num_workers=self.config.wm.rm_num_workers,
                pin_memory=True,
            )
        return dataloader, val_dataloader, loc_val_dataloader
    ...
    def _update_terminal_model(self, global_steps):
        ...
        model.train()
        dataloader, val_dataloader, loc_val_dataloader = self._build_terminal_model_dataloader(global_steps)

        f1 = self._evaluate_terminal_model(val_dataloader) #  0.7692
        if coordinator.is_master():
            print(f"before training f1: {f1:.4f}")
        if self._use_multi_resolution_reward() and loc_val_dataloader is not None:
            loc_metrics = self._evaluate_loc_reward_model(loc_val_dataloader)
            if coordinator.is_master() and loc_metrics is not None:
                print(
                    "[reward_loc] before training "
                    f"ranking_accuracy={loc_metrics['ranking_accuracy']:.4f} "
                    f"tail_negative_score={loc_metrics['tail_negative_score']:.4f} "
                    f"progress_curve={np.round(loc_metrics['progress_curve'], 4).tolist()}"
                )
        ...
        for rm_training_step, batch in enumerate(dataloader):
            ...
            if True:
                if self._use_multi_resolution_reward():
                    losses = self._compute_multi_resolution_reward_losses(model.module, batch, device)
                    ...
                    if rm_training_step != 0 and rm_training_step % 100 == 0 and coordinator.is_master():
                        ...
                        print(
                            "reward loss: "
                            f"total={avg_loss:.6f} done={float(losses['loss_done']):.6f} "
                            f"anchor={float(losses['loss_anchor']):.6f} rank={float(losses['loss_rank']):.6f} "
                            f"fail={float(losses['loss_fail']):.6f} smooth={float(losses['loss_smooth']):.6f} "
                            f"step: {rm_training_step}"
                        )
                        running_loss = 0
                else:
                    video, label, _ = batch
                ...
                if rm_training_step != 0 and rm_training_step % 100 == 0 and coordinator.is_master():
                    ...
                    if coordinator.is_master() and not self._use_multi_resolution_reward():
                ...
                if rm_training_step != 0 and rm_training_step % 500 == 0:
                    ...
                    if self._use_multi_resolution_reward() and loc_val_dataloader is not None:
                        ...
                        if coordinator.is_master() and loc_metrics is not None:
                            print(
                                "[reward_loc] "
                                f"step={rm_training_step} ranking_accuracy={loc_metrics['ranking_accuracy']:.4f} "
                                f"tail_negative_score={loc_metrics['tail_negative_score']:.4f} "
                                f"progress_curve={np.round(loc_metrics['progress_curve'], 4).tolist()}"
                            )
