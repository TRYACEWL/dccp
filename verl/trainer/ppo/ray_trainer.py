        ...
        for epoch in range(self.config.trainer.total_epochs):
            if True:
                ...
                while len(valid_batch) < batch_size * n_samples:
                    ...
                    print(
                        "[rollout-collect] "
                        f"epoch={epoch} global_step={global_steps} round={collection_rounds} "
                        f"collected={len(valid_batch)}/{batch_size * n_samples} "
                    ...
                    with Timer(name='verify', text="{name}: {seconds:.1f} seconds") as timer:
                        ...
                        print(
                            "[rollout-verify] "
                            f"round={collection_rounds} "
                            f"acc_mean={roll_batch.batch['acc'].float().mean().item():.4f} "
                            f"format_mean={roll_batch.batch['format_correctness'].float().mean().item():.4f}"
                        )
                    ...
                    kept_rollouts += len(roll_batch_to_add)
                    ...
                    print(
                        "[rollout-collect] "
                        f"after_filter_kept={len(roll_batch_to_add)} "
                        f"collected={len(valid_batch)}/{batch_size * n_samples} "
                ...
                metrics['rollout/collection_rounds'] = collection_rounds
                metrics['rollout/generated'] = generated_rollouts
                metrics['rollout/kept_after_filter'] = kept_rollouts
                metrics['rollout/target'] = batch_size * n_samples
                metrics['rollout/filter_keep_ratio'] = kept_rollouts / max(generated_rollouts, 1)
                if self.config.algorithm.adv_estimator == 'grpo':
                    metrics['grpo/n_samples'] = n_samples
                    metrics['grpo/num_prompt_groups'] = batch_size
                    metrics['grpo/target_rollouts'] = batch_size * n_samples
                    print(
                        "[grpo-collect] "
                        f"groups={batch_size} n_samples={n_samples} "
