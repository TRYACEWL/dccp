...
def main():
    ...
    if coordinator.is_master():
        ...
        if cfg.get("wandb", False):
            wandb.init(project="Open-Sora", name=exp_name, entity="2978516418-neu", config=cfg.to_dict(), dir="./outputs/wandb")
                ...
                with timers["ckpt"] as ckpt_t:
                    if ckpt_every > 0 and (global_step + 1) % ckpt_every == 0:
                        save_dir = checkpoint_io.save(
                            ...
                            model=model,
                            ema=ema,
                            optimizer=optimizer if cfg.get("save_optimizer", True) else None,
                            lr_scheduler=lr_scheduler,
                            sampler=sampler,
                            epoch=epoch,
                            step=step + 1,
