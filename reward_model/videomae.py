...
# =========================
CFG = dict(
    TRAIN_PATTERN=os.environ.get(
        "VIDEOMAE_TRAIN_PATTERN",
        "/mnt/hdfs/anonymous_user/datasets/mimicgen/iclr2025/aloha/11/20/aloha_inference/**/*.tar",
    ),
    VAL_PATTERN=os.environ.get(
        "VIDEOMAE_VAL_PATTERN",
        "/mnt/hdfs/anonymous_user/datasets/mimicgen/iclr2025/aloha/11/20/aloha_inference/**/*.tar",
    ),
    IMG_SIZE=224,
    WINDOW=8,
...
class SuccessWindowDataset(IterableDataset):
    ...
    def __init__(
        ...
        if mode == "train":
            ...
            pipeline = [
                ...
                self._windows,                      # 修正：原代码误写 _windows_train
    ...
    def _windows_val(self, stream: Iterable[Tuple[bytes, bytes]]):
        ...
        for v_bytes, m_bytes in stream:
            ...
            complete = bool(meta["complete"]) # bool(meta.get("complete", True))
            if T < W:
                continue  # 太短跳过
...
# torchrun --standalone --nproc_per_node=8 videomae.py
